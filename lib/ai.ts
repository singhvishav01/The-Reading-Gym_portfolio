...
export async function generateBookQuiz(title: string, author: string): Promise<QuizQuestion[] | null> {
  ...
  const prompt = `You are a literature expert. Generate a 3-question multiple-choice quiz about the book "${title}" by ${author}. 
The questions should test basic reading comprehension and plot knowledge to prove the user actually read it.
Return ONLY a raw JSON array. DO NOT wrap it in markdown block quotes. The array must contain exactly 3 objects with this exact structure:
